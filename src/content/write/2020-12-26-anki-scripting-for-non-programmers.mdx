...
recommendations: ['2022-05-30-anki-srs', '2018-12-30-should-i-create-a-flashcard', '2018-12-01-10-rules-for-better-flashcards']
---
...
This post revises an old post that I wrote four years ago, [Anki Scripting]({% post_url write/2016-12-26-anki-scripting %}). My motivation at that time was just to publish my notes mainly for myself. I have not anticipated that so many learners were trying to automate their flashchards creation.
...
```python showLineMarkers:{'1': [84], '2': [86], '3': [90], '4': [93], '5': [102], '6': [108]}
import os
...
col.save()
```
* <Marker number={1} /> This code references the Python module that we installed with the previous command `pip3`.
* <Marker number={2} /> This code determines the Anki directory to use. For this test, we use the path to our sandbox environnement. Make sure the path you specify really exists and use an absolute path (not a path like `./AnkiTest`).
* <Marker number={3} /> This code is equivalent to opening the Anki application.
* <Marker number={4} /> This code is equivalent to clicking on a deck on the main screen. This way, when we will create new flashcards, they will belong to this deck.
* <Marker number={5} /> This code has the same effect as clicking on the button "Add" and fill in the form.
* <Marker number={6} /> This code has the same effect as exiting the Anki application.
...
<img src="anki-new-flashcard.png" />
...
<img src="anki-compatibility.png" />
...
```python showLineMarkers:{'1': [150], '2': [176]}
# batch.py
import os
...
col.save()
```
* <Marker number={1} /> Create a variable containing all the notes to create.
* <Marker number={2} /> Iterate over this variable and create the notes as before.
...
```shell showLineMarkers:{'1': [335], '2': [336]}
...
```
* <Marker number={1} /> The CLI displays a valid command to run locally if you want to inspect the sandbox directory using the Anki Desktop application. This is a good way to validate that your flashcards have correctly been generated.
* <Marker number={2} /> The CLI displays the location of the Anki archive file containing your flashcards. This is the file to import in your collection when running Anki without option.
...
_([Download the video]({{ '/posts_resources/2020-12-26-anki-scripting-for-non-programmers/cli.mp4' | relative_url }}))_
...
_([Download the video]({{ '/posts_resources/2020-12-26-anki-scripting-for-non-programmers/import.mp4' | relative_url }}))_
