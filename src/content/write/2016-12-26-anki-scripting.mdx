---
slug: "2016/12/26/anki-scripting"
title: "Anki Scripting: Automate your flashcards"
author: "Julien Sobczak"
date: 2016-12-26
subject: "SRS"
tags: [flashcards, learning]
description: Use the Anki API to bulk the creation of your flashcards in Python.
topics: [anki]
relatedWritePosts:
...
<Admonition type="caution" className="license">
  There is a newer version of this article, [_Anki Scripting for Non-Programmers_](/write/2020/12/26/anki-scripting-for-non-programmers). This new post is shorter and is a great introduction to the subject. You can come back to this older article if you want more details about Anki and/or learn about some advanced use cases like converting an ebook into flashcards.
</Admonition>
...
<Admonition type="tip" className="None" title="Where are stored my cards?">
...
To tell Anki to use a different location, please see this [manual](https://ankisrs.net/docs/manual.html#startupopts). This could be very useful when you experiment with Anki to avoid corrupting our data!
</Admonition>
...
<Admonition type="tip" className="None" title="What is SQLite?">
...
<Admonition type="tip" className="None" title="How to inspect the Anki database?">
1. Download the SQLite DB Browser: https://sqlitebrowser.org/
2. Unzip the .apkg file that has been generated
3. Open the collection.anki2 with SQLiteBrowser (launch the executable on Windows) (be sure to consider all file extensions)
4. You should see a dialog like this:
    <img src="/posts_resources/2016-12-26-anki-scripting/sqlitebrowser-anki2-collection.png" width="750" />
</Admonition>
...
<img src="/posts_resources/2016-12-26-anki-scripting/deck-configuration.png" />
...
<Admonition type="note" className="None" title="How to inspect database changes when using Anki?">
...
<img src="/posts_resources/2016-12-26-anki-scripting/anki-home.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/anki-new_deck.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/anki-new_card.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/anki-new_reversed_card.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/anki-study.png" />

<img src="/posts_resources/2016-12-26-anki-scripting/anki-study_card.png" />
...
<Admonition type="note" title="Motivations">
...
This post will concentrate on the second option. The first option could be implemented based on the information published in the first part but is not the optimal solution (high coupling, necessity to specify all fields when most of them are irrelevant). This solution was already partially [documented online](https://decks.wikia.com/wiki/Anki_APKG_format_documentation). The Python solution is more powerful. You could use a Python module to read a PDF or an Epub and generate the associated flashcards. You could use the Google Images API to retrieve funny pictures to integrate in our flashcards. The only limit is our imagination.
...
Then, we need to download Anki dependencies. In this post, we will use [Pip](https://pypi.python.org/pypi/pip), the classic tool for installing Python packages. To avoid polluting existing Python projects you could have in local, we will use [virtualenv](https://virtualenv.pypa.io/en/stable/). The recommended way to do that in Python is to use virtualenv to isolate our project from other projects, so two projects could depend on conflicting versions of the same dependency. To install virtualenv globally, run the following command:
...
```python title="listcards.py" {'1':4} {'2':8} {'3':15}
import sys, os
...
<dl className="markers">
  ...
  <dd>We need to add the Anki project in our path</dd>

  <dt><Marker number={2} /></dt>
  <dd>We use the default installation folder of Anki on Linux. Please update the variable `PROFILE_HOME` to match your configuration.</dd>

  <dt><Marker number={3} /></dt>
  <dd>We filter the cards to only keep the card concerning the `English` language. Use `tag:*` to select all of your cards.</dd>
</dl>
...
<img src="/posts_resources/2016-12-26-anki-scripting/card-export-HTML.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/cards-export-HTML.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/card-export-img.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/app-export-html.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/generated-styled-site.png" />
...
<Admonition type="note" className="None" title="But does it not preferable to manually create the flashcards in order to retain more?">
...
The ebook is subject to a copyright, so to avoid any violation, I rewrite a short version by customizing the text. This demonstration ebook is available in the [repository associated to this post](https://github.com/julien-sobczak/anki-scripting/tree/master/anki-usecase-enidioms). To inspect its content, just unzip the archive:

<img src="/posts_resources/2016-12-26-anki-scripting/epub-unzipped.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/epub-preview.png" />

If you want to know more about the epub format, O'Reilly published whole [books](https://shop.oreilly.com/product/0636920024897.do) on the subject.
...
<img src="/posts_resources/2016-12-26-anki-scripting/english-card-type.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/note-type-fields.png" />

Close the dialog and click on *"Cards..."*. Again, we need the update the content to match the following screenshot:

<img src="/posts_resources/2016-12-26-anki-scripting/note-type-card1.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/note-type-card2.png" />
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>Load the collection from the local disk.</dd>

  <dt><Marker number={2} /></dt>
  <dd>The code reflects the Anki terminology (note, card, field, deck, tag, etc). If some term are unclear to you, check the official documentation.</dd>

  <dt><Marker number={3} /></dt>
  <dd>CSS classes defined in the note type could be used to stylize our cards. Unlike the desktop application, HTML is not escaped.</dd>

  <dt><Marker number={4} /></dt>
  <dd>The order of the fields should follow the same order as defined in the GUI.</dd>

  <dt><Marker number={5} /></dt>
  <dd>Without the explicit call to the `save` method, the flashcards would not be saved to disk. Indeed, the Anki application schedules a task every 5 minutes to call this method.</dd>
</dl>
...
```html title="40000_frequency_list_gutenberg.txt"
<table>
<thead>
<tr>
<td><b>Rank</b></td>
...
<Admonition type="note">
  Like your browser, most HTML parsers are very tolerant concerning HTML syntax. We don't need to create a perfectly valid HTML document to be able to parse it. We will continue to use the Python language, and its most popular HTML parser [BeautifulSoup](https://www.crummy.com/software/BeautifulSoup/bs4/doc/).
</Admonition>

Here is a basic program to parse this HTML file and generate a csv file:

```python title="html2csv.python"
#/usr/bin/python
...
<img src="/posts_resources/2016-12-26-anki-scripting/assistant-merge-frequency-lists.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/assistant-filter-words.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/assistant-filter-english.png" />
...
```python title="parse_wiktionary.py"
"""
...
====Synonyms====
* {{sense|electronic musical device}} {{l|en|electronic keyboard}}
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>A wiktionary entry contains the text of multiple dictionaries (English-English, English-Dutch). Only the English-English dictionary is relevant when learning the English language.</dd>

  <dt><Marker number={2} /></dt>
  <dd>A given word could be used as a noun, a verb, etc. We need to extract the definition for each type while keeping note of the type.</dd>

  <dt><Marker number={3} /></dt>
  <dd>Label syntax such as `{{t+|fr|clavier|m}}` is used exhaustively inside the wiki text. Most of the content is present inside these labels but the syntax is not easily parseable.</dd>

  <dt><Marker number={4} /></dt>
  <dd>There are many translation blocks. Often, the first one is the only translation we are interested.</dd>
</dl>
...
<img src="/posts_resources/2016-12-26-anki-scripting/wiktionary-car.png" width="750" />
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>Quotes could be on a line starting with `#:`, be surrounded with `''`</dd>

  <dt><Marker number={2} /></dt>
  <dd>While other quotes could be on a line containing a label with the attribute `passage` containing the quote text.</dd>
</dl>
...
The code behind this program contains a lot of parsing logic. The result is not the most beautiful code that I have written and I am not going to present it in this post but if you are curious, the full listing is available in the project repository in [GitHub](https://github.com/julien-sobczak/anki-scripting/blob/master/anki-usecase-enfrequency/5-parse_mytionary.py).
...
<img src="/posts_resources/2016-12-26-anki-scripting/assistant-global.png" />

Here is a screenshot of this application:

<img src="/posts_resources/2016-12-26-anki-scripting/anki-vocabulary-assistant.png" />
...
<Admonition type="note" className="None" title="Google CSE API">
...
Please visit the [official site](https://developers.google.com/custom-search/docs/tutorial/introduction) for more information.
</Admonition>

Open your browser and enter the URL http://localhost:8080. You should have the same output as the previous screenshot.

<Admonition type="caution" className="content" title="Requisites">
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>At initialization, we check the URL to determine the word to load before calling the main method `showWord`.</dd>
  <dt><Marker number={2} /></dt>
  <dd>We emit an HTTP request to retrieve the JSON containing the word information. The result is stored in the model to be accessible by the view.</dd>
</dl>
...
```java title="ApiController" {'1':17-18} {'2':22}
@RestController
@RequestMapping("/api")
public class ApiController {
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>We parse the input JSON before passing it to the constructor of the class `Word`. Like the class `Dictionary`, this class acts as a Wrapper around the raw JSON document.</dd>
  <dt><Marker number={2} /></dt>
  <dd>We use the method `save` on this object to persist the selection of the user.</dd>
</dl>

The main logic is encapsulated inside the class `Word`. Let's see its definition:

```java title="Word.java" {'1':23} {'2':24}
package fr.imlovinit.anki.vocabulary.assistant.model;
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>We download the sound from Wikipedia locally</dd>
  <dt><Marker number={2} /></dt>
  <dd>We save the JSON to disk</dd>
</dl>
...
<Admonition type="note">
  The full source of the application is available [here](https://github.com/julien-sobczak/anki-scripting/tree/master/anki-vocabulary-assistant).
</Admonition>
...
<img src="/posts_resources/2016-12-26-anki-scripting/vocabulary-note-type-fields.png" />
...
<img src="/posts_resources/2016-12-26-anki-scripting/vocabulary-note-card1.png" />
...
Check the [official Anki manual](https://ankisrs.net/docs/manual.html#conditional-replacement) to know more about the syntax used in these templates.
...
<img src="/posts_resources/2016-12-26-anki-scripting/vocabulary-note-card2.png" />
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>We need an additional field to determine if this card is required because some words have a picture but the user only want this picture to be displayed in other cards.</dd>
</dl>
...
<img src="/posts_resources/2016-12-26-anki-scripting/vocabulary-note-card3.png" />
...
The `Card 4`, `Card 5`, and `Card 6` display a sentence with a missing word and the user should find it (the same concept as [Cloze Deletion](https://ankisrs.net/docs/manual.html#cloze) in Anki):

<img src="/posts_resources/2016-12-26-anki-scripting/vocabulary-note-card4.png" />
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>Replace `SampleA` by `SampleB`, `SampleC`</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>Replace `AnswerA` by `AnswerB`, `AnswerC`</dd>
</dl>
...
<img src="/posts_resources/2016-12-26-anki-scripting/vocabulary-note-card7.png" />
...
```python title="anki-vocabulary-assistant/scripts/load_word.py"
#!/usr/bin/env python
...
- Basic programming competency is necessary to get started. The good news is that Python is the most used language in the world to introduce new people to programming. You could find many resource online (MOOC, tutorial, ebook, blog) to get started. [Khan Academy](https://www.khanacademy.org/) is a good start.
</Admonition>
