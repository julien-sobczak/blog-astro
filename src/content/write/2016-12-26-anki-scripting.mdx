...
<img src="sqlitebrowser-anki2-collection.png" />
...
```python title="listcards.py" showLineMarkers:{'1': [1127], '2': [1131], '3': [1138]}
import sys, os
...
for cid in col.findNotes("tag:English"): # <3>
    ...
    print(front)
```
* <Marker number={1} /> We need to add the Anki project in our path
* <Marker number={2} /> We use the default installation folder of Anki on Linux. Please update the variable `PROFILE_HOME` to match your configuration.
* <Marker number={3} /> We filter the cards to only keep the card concerning the `English` language. Use `tag:*` to select all of your cards.
...
```python showLineMarkers:{'1': [2133], '2': [2134], '3': [2172], '4': [2177], '5': [2196]}
def bulk_loading_anki(idioms):
    ...
    col.save() # <5>
```
* <Marker number={1} /> Load the collection from the local disk.
* <Marker number={2} /> The code reflects the Anki terminology (note, card, field, deck, tag, etc). If some term are unclear to you, check the official documentation.
* <Marker number={3} /> CSS classes defined in the note type could be used to stylize our cards. Unlike the desktop application, HTML is not escaped.
* <Marker number={4} /> The order of the fields should follow the same order as defined in the GUI.
* <Marker number={5} /> Without the explicit call to the `save` method, the flashcards would not be saved to disk. Indeed, the Anki application schedules a task every 5 minutes to call this method.
...
```
{% endraw %}
* <Marker number={1} /> A wiktionary entry contains the text of multiple dictionaries (English-English, English-Dutch). Only the English-English dictionary is relevant when learning the English language.
* <Marker number={2} /> A given word could be used as a noun, a verb, etc. We need to extract the definition for each type while keeping note of the type.
* <Marker number={3} /> Label syntax such as {% raw %}{{t+|fr|clavier|m}}{% endraw %} is used exhaustively inside the wiki text. Most of the content is present inside these labels but the syntax is not easily parseable.
* <Marker number={4} /> There are many translation blocks. Often, the first one is the only translation we are interested.
...
```
{% endraw %}
* <Marker number={1} /> Quotes could be on a line starting with `#:`, be surrounded with `''`
* <Marker number={2} /> While other quotes could be on a line containing a label with the attribute `passage` containing the quote text.
...
```html title="index.html" showLineMarkers:{'1': [3141]}
<!doctype html>
...
```
* <Marker number={1} /> We add a property `include` to `true` when the user select an element.

The HTML is self-descriptive. Let's inspect the JavaScript code used to bootstrap the AngularJS application:


```javascript title="app.js" showLineMarkers:{'2': [3178], '1': [3187]}
angular.module('app', [])
...
```
* <Marker number={1} /> At initialization, we check the URL to determine the word to load before calling the main method `showWord`.
* <Marker number={2} /> We emit an HTTP request to retrieve the JSON containing the word information. The result is stored in the model to be accessible by the view.
...
```java title="ApiController" showLineMarkers:{'1': [3261], '2': [3265]}
@RestController
@RequestMapping("/api")
public class ApiController {
...
```
* <Marker number={1} /> We parse the input JSON before passing it to the constructor of the class `Word`. Like the class `Dictionary`, this class acts as a Wrapper around the raw JSON document.
* <Marker number={2} /> We use the method `save` on this object to persist the selection of the user.

The main logic is encapsulated inside the class `Word`. Let's see its definition:

```java title="Word.java" showLineMarkers:{'1': [3300], '2': [3301]}
package fr.imlovinit.anki.vocabulary.assistant.model;
...
```
* <Marker number={1} /> We download the sound from Wikipedia locally
* <Marker number={2} /> We save the JSON to disk
...
```
{% endraw %}
* <Marker number={1} /> We need an additional field to determine if this card is required because some words have a picture but the user only want this picture to be displayed in other cards.
...
```
{% endraw %}
* <Marker number={1} /> Replace `SampleA` by `SampleB`, `SampleC`
...
```
{% endraw %}
* <Marker number={1} /> Replace `AnswerA` by `AnswerB`, `AnswerC`
