---
slug: "2021/08/10/linux-system-calls-under-the-hood"
...
<Admonition type="note" className="None" title="Library Function vs System Call">
...
<img src="/posts_resources/2021-08-10-linux-system-calls-under-the-hood/system-calls.png" />
</Admonition>

For example, in C, the PID of your program is accessible using the function `getpid()`:

```c title="getpid.c" showLineMarkers:{'1': [95]}
#include <stdio.h>
#include <sys/types.h>
#include <unistd.h>
...
```
* <Marker number={1} /> The `getpid()` library function is defined in `unistd.h` and returns the PID of the current process using the `getpid` system call.
...
```c title="benchmark.c" showLineMarkers:{'1': [114], '2': [126, 133], '3': [134]}
#include <stdio.h>
#include <sys/types.h>
#include <unistd.h>
#include <time.h>
...
```
* <Marker number={1} /> We defined a basic function returning an integer literal. Retrieving the PID of a process doesn't interact with a hardware device. Simply returning this integer is relatively close to reading this value from a data structure kept in memory.
* <Marker number={2} /> We call the two functions 1,000,000 times and measure how long it takes.
...
<Admonition type="note" className="None" title="API vs ABI">
...
```c title="sysdeps/unix/sysv/linux/x86_64/sysdep.h"
...
```c title="/sysdeps/unix/sysv/linux/getpid.c" showLineMarkers:{'1': [449], '2': [451], '3': [453]}
#include <unistd.h>
#include <tls.h>
#include <sysdep.h>
...
```
* <Marker number={1} /> The function `__getpid()` is implemented as a C function.
* <Marker number={2} /> The code checks for the `pid` in the thread-local memory area to determine if the function has already been called.
* <Marker number={3} /> If the cache is empty, the code delegates to `really_getpid()` that checks the cache again before calling the macro `INTERNAL_SYSCALL` we have just covered before.
...
```
```
* <Marker number={1} /> No rule is generated.
...
```c title="kernel/sys.c" showLineMarkers:{'1': [517]}
/**
...
```
* <Marker number={1} /> The code uses the `current` pointer representing the current task, which is issuing the syscall. The PID is then extracted from this struct. We will not cover it further.
...
```c title="include/linx/syscalls.h"
/* kernel/sys.c */
...
```c title="include/uapi/asm-generic/unistd.h"
/* kernel/sys.c */
...
...
```
* <Marker number={1} /> We find again the number 39 representing the `getpid` system call on `amd64`.
...
.`int 0x80` vs `syscall`
<Admonition type="note" className="None" title="sysd-syscalls">
...
```c title="arch/x86/kernel/cpu/common.c" showLineMarkers:{'1': [607], '2': [608]}
...
```
* <Marker number={1} /> `MSR_*` are [Model-specific Register](https://en.wikipedia.org/wiki/Model-specific_register) and can only be written by the privileged CPU instruction `wrmsr`. This first line is low-level code to ensure that we return to user code with the related privilege.
* <Marker number={2} /> `entry_SYSCALL_64` is the system call entry. We store the address of this function.
...
```c title="arch/x86/entry/syscall_64.c" showLineMarkers:{'1': [629], '2': [630]}
asmlinkage const sys_call_ptr_t sys_call_table[__NR_syscall_max+1] = {
	[0 ... __NR_syscall_max] = &__x64_sys_ni_syscall, // <1>
  #include <asm/syscalls_64.h> // <2>
};
```
* <Marker number={1} /> All elements point initially to the `sys_ni_syscall` function, which is a fallback function simply returning `-ENOSYS` (`Function not implemented`).
* <Marker number={2} /> The header file `asm/syscalls_64.h` is generated dynamically from the list of system calls on your system and overrides the default handler for all defined system calls.

This `asm/syscalls_64.h` file is generated by the script `arch/x86/entry/syscalls/syscalltbl.sh` and the result looks like:

```c title="asm/syscalls_64.h"
...
```c title="arch/x86/entry/entry_64.S"
.code64
...
```c title="arch/x86/entry/common.c" showLineMarkers:{'1': [731], '2': [732], '3': [733]}
...
```
* <Marker number={1} /> Check the system call number is valid. The value of `NR_syscalls` is determined at compile time.
* <Marker number={2} /> Clamp the index within `[0..NR_syscalls]`.
* <Marker number={3} /> Execute the function present in the system call table with the specified number.
...
<img src="/posts_resources/2021-08-10-linux-system-calls-under-the-hood/system-call-steps.png" />





<Admonition type="note" className="None" title="Try It Out!">
...
<img src="/posts_resources/2021-08-10-linux-system-calls-under-the-hood/compilation.png" />
...
```
* <Marker number={1} /> We retrieve the initial address `0x401000` as reported previously by the command `objdump`.
...
```go title="src/syscall/zsycall_linux_amd64.go"
...
```go title="src/syscall/syscall_linux.go"
...
...
```asm title="src/cmd/vendor/golang.org/x/sys/unix/asm_linux_amd64.s"
...
```go title="src/cmd/vendor/golang.org/x/sys/unix/zsysnum_linux_amd64.go"
SYS_GETPID                 = 39
```

And now, the native implementation of the `getpid` system call:

```asm title="src/runtime/sys_linux_amd64.s"
...
```java showLineMarkers:{'1': [1039]}
long pid = ProcessHandle.current().pid(); // <1>
```
* <Marker number={1} /> We grab a handle to the current process and then query the PID.
...
```java title="java/lang/ProcessHandleImpl.java"
package java.lang;
...
```c title="src/java.base/unix/native/libjava/ProcessHandleImpl_unix.c" showLineMarkers:{'1': [1110]}
#include "jni.h"
#include "jni_util.h"
...
```
* <Marker number={1} /> The [function `getpid()`](https://linux.die.net/man/3/getpid) is defined by glibc.
...
.To Remember
<Admonition type="note" className="remember" title="src/syscall/zsycall_linux_amd64.go">
