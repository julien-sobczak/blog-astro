...
<img src="/posts_resources/2021-08-10-linux-system-calls-under-the-hood/system-calls.png" title="System calls are the border between your application and the Linux kernel" width="500" />
</Admonition>
...
<dl className="markers">
  ...
  <dd>The `getpid()` library function is defined in `unistd.h` and returns the PID of the current process using the `getpid` system call.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>We defined a basic function returning an integer literal. Retrieving the PID of a process doesn't interact with a hardware device. Simply returning this integer is relatively close to reading this value from a data structure kept in memory.</dd>

  <dt><Marker number={2} /></dt>
  <dd>We call the two functions 1,000,000 times and measure how long it takes.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>The function `__getpid()` is implemented as a C function.</dd>

  <dt><Marker number={2} /></dt>
  <dd>The code checks for the `pid` in the thread-local memory area to determine if the function has already been called.</dd>

  <dt><Marker number={3} /></dt>
  <dd>If the cache is empty, the code delegates to `really_getpid()` that checks the cache again before calling the macro `INTERNAL_SYSCALL` we have just covered before.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>The code uses the `current` pointer representing the current task, which is issuing the syscall. The PID is then extracted from this struct. We will not cover it further.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>We find again the number 39 representing the `getpid` system call on `amd64`.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>`MSR_*` are [Model-specific Register](https://en.wikipedia.org/wiki/Model-specific_register) and can only be written by the privileged CPU instruction `wrmsr`. This first line is low-level code to ensure that we return to user code with the related privilege.</dd>

  <dt><Marker number={2} /></dt>
  <dd>`entry_SYSCALL_64` is the system call entry. We store the address of this function.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>All elements point initially to the `sys_ni_syscall` function, which is a fallback function simply returning `-ENOSYS` (`Function not implemented`).</dd>
  <dt><Marker number={2} /></dt>
  <dd>The header file `asm/syscalls_64.h` is generated dynamically from the list of system calls on your system and overrides the default handler for all defined system calls.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>Check the system call number is valid. The value of `NR_syscalls` is determined at compile time.</dd

  <dt><Marker number={2} /></dt>
  <dd>Clamp the index within `[0..NR_syscalls]`.</dd

  <dt><Marker number={3} /></dt>
  <dd>Execute the function present in the system call table with the specified number.</dd
</dl>
...
<img src="/posts_resources/2021-08-10-linux-system-calls-under-the-hood/system-call-steps.png" title="Steps when executing a system call" width="650" />
...
<img src="/posts_resources/2021-08-10-linux-system-calls-under-the-hood/compilation.png" width="600" />
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>We retrieve the initial address `0x401000` as reported previously by the command `objdump`.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>We grab a handle to the current process and then query the PID.</dd>
</dl>
...
<dl className="markers">
  <dt><Marker number={1} /></dt>
  <dd>The [function `getpid()`](https://linux.die.net/man/3/getpid) is defined by glibc.</dd>
</dl>
