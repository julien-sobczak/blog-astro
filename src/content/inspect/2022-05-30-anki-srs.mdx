---
slug: "2022/05/30/anki-srs"
title: "Anki SRS Algorithm"
author: "Julien Sobczak"
date: 2022-05-30
subject: "Anki"
tags: ['learning']
unofficialTags: ['flashcards']
excerpt: >
...
>
> -- <cite> Hermann Ebbinghaus, German Psychologist having introduced the Forgetting Curve</cite>
...
The Anki source code is published under [AGPL v3](https://www.gnu.org/licenses/agpl-3.0.en.html). The code presented in this article has been slightly adapted for learning and readability purposes.
...
<img src="/posts_resources/2022-05-30-anki-srs/forgetting-curve.png" />
...
If [Hermann Ebbinghaus](https://en.wikipedia.org/wiki/Hermann_Ebbinghaus) is credited for the initial research behind SRS, the [Leiter System](https://en.wikipedia.org/wiki/Leitner_system) is often quoted as the first algorithm. This system uses a physical box as depicted by the following illustration:

<img src="/posts_resources/2022-05-30-anki-srs/Leitner-original.png" />
...
<img src="/posts_resources/2022-05-30-anki-srs/Leitner-modern.png" />
...
The [SM-0 algorithm](http://super-memory.com/articles/paper.htm) (aka the paper-and-pencil SuperMemo method) was published in 1985 and relies on [paper books filled with tables](http://super-memory.com/articles/paper.htm).

<img src="/posts_resources/2022-05-30-anki-srs/sm-0.png" />

Although the algorithm was designed to be executed manually, we can still capture the logic using code:

```python showLineMarkers:{'1': [268]}
import random
...
```
* <Marker number={1} /> The grade of the answer does not influence the next interval. Difficult items are reviewed again the same day but the next intervals are fixed and determined with a factor 1.7 when creating the page.
...
Unlike physical systems where cards are grouped in the same box/partition/page and are reviewed collectively, digital systems consider each item separately. For example, the SuperMemo algorithm called [SM-2](https://www.supermemo.com/en/archives1990-2015/english/ol/sm2) assigns a specific level of difficulty to every card and determines the appropriate intervals between repetitions using this specific value (called the E-Factor).

<img src="/posts_resources/2022-05-30-anki-srs/sm-2.png" />

Now, the same logic but implemented as code:

```python showLineMarkers:{'1': [345], '2': [352], '3': [358], '4': [388]}
import random
from datetime import date, timedelta
from queue import Queue
    ...
    for i in range(365):
        ...
        while not items_to_review.empty():
            ...
            if not item.review(day, q): # <4>
                items_to_review.put(item)
```
* <Marker number={1} /> The E-Factor never goes down lower than 1.3. SuperMemo found out that items having lower E-Factors were repeated annoyingly often when the root cause was usually their formulation and not the review process. We will see how Anki manages such cards later. These items must often be reformulated to conform to the [minimum information principle](https://supermemo.guru/wiki/Minimum_information_principle).
* <Marker number={2} /> The E-Factor is always initialized to the same difficulty value. It will decrease for bad grades and increase for good grades.
* <Marker number={3} /> Unlike SM-0, the grades (= item difficulty) influence the factor used to determine the next interval.
* <Marker number={4} /> Like SM-0, difficult items are reviewed again the same day.
...
From [Wikipedia](https://en.wikipedia.org/wiki/Anki_(software)):
...
Anki source code includes different versions of its SRS algorithm (called _Scheduler_). All got inspiration from SM-2. The [V2](https://faqs.ankiweb.net/the-anki-2.1-scheduler.html) is in use since 2018 even if the [V3](https://faqs.ankiweb.net/the-2021-scheduler.html) is looming. For this article, we can ignore the details between these versions. Check the source code on GitHub if you are interested in the differences between the [V1](https://github.com/ankitects/anki/blob/2.1.49/pylib/anki/scheduler/v1.py), [V2](https://github.com/ankitects/anki/blob/2.1.49/pylib/anki/scheduler/v2.py), or [V3](https://github.com/ankitects/anki/blob/2.1.49/pylib/anki/scheduler/v3).


We will analyze the V2.1 scheduler as it is the version I'm familiar with. We will use the [version 2.10.0](https://github.com/ankitects/anki/blob/2.1.10/anki/schedv2.py) of Anki Desktop to ignore recent refactorings (the rewrite of backend code in Rust, the introduction of Protocol Buffer messages, the factorization of common code among scheduler versions using inheritance, etc.). This will help us keep the code easy to grasp.

Here is a recall of [Anki terminology](https://docs.ankiweb.net/getting-started.html#key-concepts):

<img src="/posts_resources/2022-05-30-anki-srs/anki-terminology.png" />
...
<img src="/posts_resources/2022-05-30-anki-srs/anki-workflow.png" />

Here is an overview of the Anki algorithm:

<img src="/posts_resources/2022-05-30-anki-srs/anki-srs.png" />

The use of separate new/review queues tries to remediate a common complaint with the standard SM-2 algorithm is that repeated failings of a card cause the card to get stuck in "low interval hell" (also known as "ease hell"). [In Anki](https://faqs.ankiweb.net/what-spaced-repetition-algorithm.html#review-cards), the initial acquisition process does not influence the ease factor.
...
```python showLineMarkers:{'1': [447], '2': [452, 458], '3': [464]}
...
```
* <Marker number={1} /> If there is no more card to review now but the next card in learning is in less than `collapseTime` seconds, show it now. +
...
* <Marker number={2} /> The settings differ based on the queue where a card belongs. For example, when learning (`new`) cards, the delay is increased by graduating steps whereas the delay is multiplied by a given factor for review (`rev`) cards. The meaning of individual settings will become clearer when we will detail the logic. +
...
* <Marker number={3} /> When you forget a review card, it is said to have "lapsed", and the card must be relearnt. The default behavior for lapsed reviews is to reset the interval (`minInt`) to 1 (i.e. make it due tomorrow) and put it in the learning queue for a refresher (`delays`) in 10 minutes. +
...
```python showLineMarkers:{'1': [519], '2': [527, 537]}
class Collection:
...
class Card:
    ...
    def __init__(self, note, id=None):
        ...
        self.due = self.id
```
* <Marker number={1} /> The `Scheduler` implementation will be the main topic of the remaining of this section.
* <Marker number={2} /> The identifiers are initialized using a helper function `intId()` which uses the current time and ensures two successive calls return different values. Here is the definition:
...
```python showLineMarkers:{'1': [610], '2': [612], '3': [614]}
class Scheduler:
    ...
    def __init__(self, col):
        ...
        self.reset()
```
* <Marker number={1} /> The attribute `today` represents the number of days since the collection creation. It is used when searching for review cards where the attribute `due` represents the number of days relative to it. The value is initialized like this:
+
```python
class Scheduler:
    ...
    def _daysSinceCreation(self):
        ...
        # Note: 86400s = 1d
```
* <Marker number={2} /> The attribute `dayCutoff` represents the timestamp of the beginning of the next day. Anki allows customizing at which hour a day ends. Here, we simply use midnight:
+
```python
class Scheduler:
    ...
    def _dayCutoff(self):
        ...
        return stamp
```
* <Marker number={3} /> The attribute `_lrnCutoff` is related to the setting `collapseTime` (also called the learn ahead limit). The method `_updateLrnCutoff()` is used to initialize it and update it:
+
```python
class Scheduler:
...
```python showLineMarkers:{'1': [674, 707, 723], '2': [675], '3': [682], '4': [714], '5': [730]}
class Scheduler:
...
```
* <Marker number={1} /> By default, the queues are empty. Anki defers their filling until a card is retrieved.
...
* <Marker number={3} /> Anki searches for all cards in the queue `0` (= `new`) and sorts them by their due date before returning the first N cards based on the current daily limit.
* <Marker number={4} /> Anki searches for all cards in the queue `1` (= `lrn`) that are due and sorts them by timestamp as the id is initialized from the creation timestamp
* <Marker number={5} /> Anki searches for all cards in the queue `2` (= `rev`) that are due and sorts them by the due date before returning the first N shuffled cards based on the current daily limit.
...
```python showLineMarkers:{'1': [771], '2': [803, 822, 829]}
class Scheduler:
...
```
* <Marker number={1} /> By default, Anki shows cards in a well-defined order:
+
...
* <Marker number={2} /> The methods `_fillXXX()` return `True` when a queue is not empty, in which case, we simply have to pop an element from it.
...
```python showLineNumbers highlight='4,9-14,16-19,22' showLineMarkers:{'1': [848], '2': [866]}
class Scheduler:
...
```
* <Marker number={1} /> The method `_updateCutoff()` is called every time the queues are reset (= once a day). When this happens, it means a new day began and therefore the day limit must be refreshed too.
* <Marker number={2} /> The method `_checkDay()` is called every time we retrieve a new card to study. This way, if we have passed the current day, the queue will be reset before returning the next card.
...
```python showLineMarkers:{'1': [912], '2': [913], '3': [915]}
class Scheduler:
    ...
    def _leftToday(self, delays, left, now=None):
        ...
        return ok+1
```
* <Marker number={1} /> Anki simply updates the attribute `queue` to move a card to a different queue. When the destination queue will be reset (ex: for tomorrow's session), the card will be automatically inserted into it.
...
* <Marker number={3} /> The attribute `left` is particular. The numeric format keeps two pieces of information: how many times the card will be reviewed today, and how many steps before graduation. The methods `_startingLeft` and `_leftToday` implement this logic. You can safely ignore the details.
...
```
* <Marker number={1} /> The settings differ according to if the card comes from the review or new queue. For example, the steps are different after a lapse than when learning a new card for the first time.
...
```python showLineMarkers:{'1': [993], '2': [997], '3': [999], '4': [1017]}
class Scheduler:
    ...
    def _delayForGrade(self, conf, left): # <4>
        ...
        return delay*60
```
* <Marker number={1} /> We restore the attribute `left` as if the card were new.
* <Marker number={2} /> We process lapses differently. By default, we reset the attribute `ivl` to `1` (next review in one day).
* <Marker number={3} /> The card due date is determined by adding the next step to the current date. The card remains in the learning queue (`1`).
* <Marker number={4} /> The method `_delayForGrade()` is a helper method to get the next step interval. The method extracts the number of remaining steps from the attribute `left` (Ex: `1002` => 2 remaining steps) and uses the setting `delay` to find the matching delay (Ex: `1m 10m 1d` => next study in `10m`).
...
class Scheduler:
    ...
    def _delayForRepeatingGrade(self, conf, left):
        ...
        return avg
```
* <Marker number={1} /> We reuse the method `_rescheduleLrnCard()` introduced just before to update the card's due date.
...
```python showLineMarkers:{'1': [1078], '2': [1080], '3': [1095]}
class Scheduler:
    ...
    def _graduatingIvl(self, card, conf, early):
        ...
        return ideal
```
* <Marker number={1} /> When a lapse is graduated, we add the previous interval to the current date to determine the due date.
* <Marker number={2} /> When a new card is graduated, we initialize the two key attributes relative to the SRS algorithm: the ease factor and the interval. These fields will be necessary to determine the next due date for review cards.
* <Marker number={3} /> When graduating a new card, the initial interval will be different if we are completed all steps ("Good") or if we have pressed ("Easy") to immediately graduate the card (1 vs 4 days by default).

_Case 2_: If there are remaining steps:

```python showLineMarkers:{'1': [1116]}
class Scheduler:
...
```
* <Marker number={1} /> The attribute `left` is updated to decrement the number of remaining steps and to recalculate the number of studies until the next day.
...
```python showLineMarkers:{'1': [1154], '2': [1155], '3': [1157], '4': [1161], '5': [1164]}
class Scheduler:
    ...
    def _checkLeech(self, card, conf):
        if card.lapses >= conf['leechFails']:
            ...
            return True
```
* <Marker number={1} /> The number of lapses for this card is increased.
* <Marker number={2} /> The ease factor is reduced by 0.2 (but no lower than 1.3 as recommended by SM-2).
* <Marker number={3} /> If the number of lapses reaches the value of the setting `leechFails`, the card is marked as a leech. A tag is added to the note and the card is moved to the queue `-1` (= suspended). The card will therefore be ignored when filling the different queue as no method `_fillXXX()` considers cards in the queue `-1`.
...
```python showLineMarkers:{'1': [1195], '2': [1198], '3': [1207]}
class Scheduler:
    ...
    def _constrainedIvl(self, ivl, conf, prev):
        ...
        return int(ivl)
```
* <Marker number={1} /> The attribute `ivl` determines the next due date (we add it to the current date to determine the value of the attribute `due`).
* <Marker number={2} /> The ease factor is changed by removing 0.15 for "Hard" cards or by adding 0.15 for "Easy" cards. The ease factor is left unchanged for "Good" cards. Only their intervals will be changed to increase the period between studies.
* <Marker number={3} /> The method `_nextRevIvl()` determine the next interval:
...
**We are done** 🎉. The complete code is available in the [companion GitHub repository](https://github.com/julien-sobczak/anki-srs-under-the-hood/blob/main/anki/schedv2_minimal_v3.py). A more complete [annotated version](https://github.com/julien-sobczak/anki-srs-under-the-hood/blob/main/anki/schedv2_annotated.py) is also available in the same repository including two additional features described next.



### Bonus: Day Boundaries

[Anki treats small steps and steps that cross a day boundary differently](https://docs.ankiweb.net/deck-options.html?#day-boundaries). With small steps, the cards are shown as soon as the delay has passed, in preference to other due cards in review. This is done so that you can answer the card as closely to the calculated delay as possible. In contrast, if the interval crosses a day boundary, it is automatically converted to days.

In the implementation, the code splits the learning queue into two distinct queues: sub-day learning and day learning.

```python showLineMarkers:{'1': [1262], '2': [1273], '3': [1286, 1302]}
# ...
...
def _getCard(self):
    ...
    return self._getLrnCard(collapse=True)
```
* <Marker number={1} /> The previous queue is split into two queues:
...
* <Marker number={3} /> Sub-day learning cards are prioritized first to be sure to review them as close as their delay in seconds. Day learning cards are reviewed last since their delay in days tolerates more flexibility (reviewing them the next day is not as bad as for sub-day learning cards).


### Bonus: Fuzzing

When you select an ease button on a review card, [Anki also applies a small amount of random “fuzz”](https://docs.ankiweb.net/studying.html#fuzz-factor) to prevent cards that were introduced at the same time and given the same ratings from sticking together and always coming up for review on the same day.

Here is the code:

```python showLineMarkers:{'1': [1331], '2': [1340, 1342, 1344]}
...
```
* <Marker number={1} /> The function `_fuzzedIvl()` is only called for intervals greater than one day. For sub-day learning cards introduced in the previous point, fuzzing is also applied up to 5 minutes:
+
```python
maxExtrax = min(300, int(delay*0.25))
fuzz = random.randrange(0, maxExtra)
```
* <Marker number={2} /> The fuzz factor is reduced but the fuzzing increases as intervals become larger.



## A Better Anki SRS Algorithm?

The [SM-2 algorithm](http://www.supermemo.com/english/ol/sm2.htm), on which Anki is based, was released in 1987 in SuperMemo 1.0. It was revised several times since:

* [SM-4](http://super-memory.com/english/ol/sm4.htm) (1989)
* [SM-5](http://super-memory.com/english/ol/sm5.htm) (1989)
* [SM-6](http://super-memory.com/english/ol/sm6.htm) (1991)
* [SM-8](http://super-memory.com/english/algsm8.htm) (1995)
* [SM-11](http://super-memory.com/english/algsm11.htm) (2002)
* [SM-15](http://super-memory.com/english/algsm15.htm) (2011)
* [SM-17](https://supermemo.guru/wiki/Algorithm_SM-17) (2015)
* [SM-18](https://supermemo.guru/wiki/Algorithm_SM-18) (2019)

Each version iterates over deficiencies of the previous one.
You can find a [short summary](https://supermemo.guru/wiki/History_of_SuperMemo_algorithm) of the main changes or a (very) [long summary](https://www.supermemo.com/fr/articles/history) of the history of SuperMemo. The short version is probably too terse to understand the improvements, and the long version is probably too detailed to understand everything. (It took me more than 5 hours to read it but it was worth the reading!)
...
>
> -- <cite> Piotr Wozniak, Original author of SuperMemo</cite>
...
<img src="/posts_resources/2022-05-30-anki-srs/sm-5-matrix.png" />
...
The two-component model of long-term memory still represents the foundation of SuperMemo since its introduction in SM-4 in 1989. [Piotr Wozniak was pessimistic about a better, faster, and more effective algorithm](http://super-memory.com/articles/kowal.htm) as soon as 1994. The versions of the algorithm that appeared after that didn't introduce a breakthrough improvement like SuperMemo did when it abandoned the SM-2 algorithm in 1989, the same algorithm that keeps popping up in new applications.


<Admonition type="note" className="None" title={`"New" Applications`}>
Many applications relying on SRS appeared in popular app stores more or less recently: [Quizlet](https://quizlet.com/), [Memrise](https://www.memrise.com/), [Duolingo](https://www.duolingo.com/), [LingoDeer](https://www.lingodeer.com/), [Brainscape](https://www.brainscape.com/), [Lingvist](https://lingvist.com/), [Chegg](https://chegg.com), [RemNote](https://www.remnote.com/), [Mochi](https://mochi.cards/), [Memcode](https://www.memcode.com/), ...

* [__Mochi__'s algorithm](https://mochi.cards/docs/#spaced-repetition) is very simple. The card interval is doubled after each correct answer, and cut in half otherwise.

* [__Memrise__'s algorithm](https://memrise.zendesk.com/hc/en-us/articles/360015889057-How-does-the-spaced-repetition-system-work-) is similar to _Mochi_'s. The card interval increases using the following steps: 4 hours, 12 hours, 24 hours, 6 days, 12 days, 48 days, 96 days, and 6 months. Any wrong answer moves back the card to the first interval.

* [Quizlet's algorithm](https://quizlet.com/blog/spaced-repetition-for-all-cognitive-science-meets-big-data-in-a-procrastinating-world) has known several iterations. The first implementation simply repeats all the questions you got wrong. The second implementation is similar to Anki where the card interval increases by approximately 2.2 and wrong answers reset the interval to one day. The next implementation relies on machine learning and uses the millions of answers to determine the recall probability, which is the chance you answer correctly. This allows, for example, to reduce the interval for words with irregular spellings when learning a foreign language.

* [__Duolingo__'s algorithm](https://blog.duolingo.com/how-we-learn-how-you-learn/) is similar to Quizlet. Duolingo has millions of students who generate billions of statistics about language learning every day. Like Quizlet, Duolingo uses machine learning to predict how likely you are to remember any given word at any time. This is represented by the strength meter (still strong, pretty good, time to practice, overdue) below every lesson.

* [RemNote's algorithm](https://www.youtube.com/watch?v=IwaoV-C9az8&ab_channel=RemNote) is customizable like Anki and most settings will look familiar to Anki users, especially after following this tutorial.

* [Memcode's algorithm](https://news.ycombinator.com/item?id=23948541) also uses SM-2.
</Admonition>
...
* The [Anki Website](https://faqs.ankiweb.net/what-spaced-repetition-algorithm.html) explains succinctly the main differences between its algorithm and SM-2.
* [Anki Database Structure](https://github.com/ankidroid/Anki-Android/wiki/Database-Structure): The most up-to-date guide to the Anki internal database schema, which was more than useful during the writing of this article.
* [The Ease Factor Problem](https://web.archive.org/web/20200926103540/https://massimmersionapproach.com/table-of-contents/anki/low-key-anki/the-ease-factor-problem/): Interesting insight about the impact of changing the ease factor after a lapse.
* [A great video](https://www.youtube.com/watch?v=1XaJjbCSXT0) to introduce most of the notions covered in the Anki section.
* Last but not least, [the true history of spaced repetition](https://www.supermemo.com/en/articles/history): An extensive coverage of the subject by Piotr Wozniak. A reference.
...
> [Mochi's algorithm](https://mochi.cards/docs/#spaced-repetition) is very simple. Each time you remember a card, the time it takes before it's reviewed again doubles. When a card is forgotten that time is cut in half. These settings can be tweaked in the settings, and different decks can have their own settings.
>
> -- <cite> Mochi Documentation</cite>
...
>
> -- <cite> Quizlet Documentation</cite>


### UI

<img src="anki-preferences-scheduling.png" />
...
<img src="anki-deck-options-scheduling.png" />
