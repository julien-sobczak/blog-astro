...
```javascript showLineMarkers:{'1': [37], '2': [57], '3': [61]}
var Prism = (function() {
...
```
* <Marker number={1} /> This is the entry point. We look for all source code present in the document.
* <Marker number={2} /> We replace the previous content with the stylized one.
* <Marker number={3} /> We tokenize the source code to colorize it using CSS classes.
...
```javascript showLineNumbers highlight='10,26..28,31..32'
tokenize : function(text, grammar) {
 var strarr = [ text ];
