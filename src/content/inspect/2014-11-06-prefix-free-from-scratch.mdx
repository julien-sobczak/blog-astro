...
```html showLineNumbers highlight='8..13' showLineMarkers:{'1': [78]}
...
```
* <Marker number={1} /> The stylesheet has disappeared and has been replaced by an inline stylesheet. The content is identical with one exception: the use of the prefix `-moz` for the still unsupported property `border-radius`.
...
<Admonition type="tip" className="None" title="Zoom on `querySelectorAll`">
...
```javascript showLineMarkers:{'1': [229], '2': [230], '3': [233]}
StyleFix.register(function(css, raw, element) {
 var prefix = '-moz-', // <1>
...
```
* <Marker number={1} /> We focus on Firefox 3.6 for now.
* <Marker number={2} /> We consider only the property `border-radius`.
* <Marker number={3} /> We search for every property to replace.
...
```javascript showLineMarkers:{'1': [263]}
