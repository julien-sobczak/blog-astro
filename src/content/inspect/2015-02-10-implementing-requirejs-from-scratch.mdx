...
<img src="/posts_resources/2015-02-10-implementing-requirejs-from-scratch/demo-output.png" />
...
<img src="/posts_resources/2015-02-10-implementing-requirejs-from-scratch/demo-requests.png" />
...
```html showLineMarkers:{'1': [194], '2': [195]}
<!DOCTYPE html>
<html>
    <head>
        <title>My Sample Project</title>
...
```
* <Marker number={1} /> We import jQuery, not indispensable but convenient to reuse some utility functions.
* <Marker number={2} /> We replace the library RequireJS with a new file `require.lite.js` that we will be the subject of this article.
...
```javascript showLineMarkers:{'1': [274], '2': [276], '3': [279], '4': [283]}
...
```
* <Marker number={1} /> `depExports` will contain the arguments that will be passed to the instantiation callback of this module. After the loading of a dependency, we save the value in this array.
* <Marker number={2} /> The instantiation callback must execute only when all its dependencies have been loaded. Using this counter, we know the number of dependencies that are still not completely loaded.
* <Marker number={3} /> It is important to assign an id even for simple files containing a simple call to `require`. If this file is referenced several times, it will be loaded only once.
...
```javascript showLineMarkers:{'1': [351], '2': [355], '3': [362], '4': [364], '5': [378]}
Module.prototype = {
...
```
* <Marker number={1} /> As for the method `init`, we memorize that the module has been activated. It prevents the module from being initialized twice.
* <Marker number={2} /> We load dependencies transitively. We can enable the module before all its dependencies have been enabled first. We iterate over them, and if the dependency is new  (this is the motivation behind the variable `registry`), we instantiate its `Module`, before trying its activation (recursive method).
* <Marker number={3} /> The property `depCount` is incremented to indicate that we are waiting for the loading of this module.
* <Marker number={4} /> We register to decrement this variable when the module has been defined.
* <Marker number={5} /> There are two calls to the method `check`: at the end of our own activation, and for each dependency definition. Here is the code of this method `check`:
...
```javascript showLineMarkers:{'1': [515], '2': [523], '3': [528]}
var defQueue = [];
...
```
* <Marker number={1} /> Not a string in first argument, we know this is an anonymous module, we shift the arguments in consequence.
* <Marker number={2} /> Not an array, we know that the module has no dependency. We shift the arguments again.
* <Marker number={3} /> Variables contains now the right value.
...
```javascript showLineMarkers:{'1': [554], '2': [577], '3': [588]}
/**
...
```
* <Marker number={1} /> We trigger the callback on the event `load`.
* <Marker number={2} /> We traverse saved values until finding our module.
* <Marker number={3} /> We complete the module initialization.
