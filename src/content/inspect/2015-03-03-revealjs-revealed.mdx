...
```javascript showLineMarkers:{'1': [116], '2': [121, 122], '3': [134]}
var Reveal;
...
```
* <Marker number={1} /> We predefine main CSS selectors to retrieve all slides or just the horizontal and vertical ones. These constants will be reused several times during the next steps.
* <Marker number={2} /> We define two variables `indexh` and `indexv` to represent our current position inside the presentation, like a slide number.
* <Marker number={3} /> We end by requesting the display of the first slide (i.e., the first horizontal and vertical slide). For the moment, everything remains to be implemented in this method.
...
<Admonition type="tip" className="None" title="Zoom on `element.classList`">
...
```javascript showLineMarkers:{'1': [326]}
/**
...
```
* <Marker number={1} /> We compare the default size for a slide (960x700) with the effective screen size. We obtain the ratio to apply to scale the slide to match the full screen.

Let's modify the method `slide` to use this new method:

```javascript showLineNumbers highlight='7'
function slide( h, v ) {
...
```javascript showLineNumbers highlight='8'
function initialize() {
...
<img src="slides-with-layout.png" />
...
```javascript showLineNumbers highlight='34'
/**
...
```javascript showLineMarkers:{'1': [443, 444, 445, 446]}
/**
...
```
* <Marker number={1} /> We use the two variables `indexh` and `indexv` to determine our current position, before calling the method `slide` to move in the right direction.
...
```javascript showLineNumbers highlight='24,32,40,48'
/**
...
```css showLineMarkers:{'1': [553], '2': [556, 559, 562, 565]}
...
.reveal .slides > section > section.future {
  transform: translate(0, 150%); } // <2>
```
* <Marker number={1} /> We configure the animation to start slowly.
* <Marker number={2} /> We rely on the function [`translate`](https://developer.mozilla.org/fr/docs/Web/CSS/transform#translate). Using a large percent, we are sure the slide will completely exit the screen.
...
<Admonition type="tip" className="None" title="Zoom on function `cubier-bezier`">
...
The website [cubic-bezier.com](http://lea.verou.me/2011/09/a-better-tool-for-cubic-bezier-easing/) allows you to [create visually your curve](http://cubic-bezier.com/#.26,.86,.44,.98) and generates the corresponding CSS code. This site is the work of Lea Verou, to whom we also owe the projects [-prefix-free]({% post_url inspect/2014-11-06-prefix-free-from-scratch %}) and [prism]({% post_url inspect/2014-11-25-prism-from-scratch %}).
</Admonition>
...
```javascript showLineNumbers highlight='6,28'
/**
 * Binds all event listeners.
 */
function addEventListeners() {
...
```javascript showLineMarkers:{'1': [683]}
/**
 * Starts up the presentation.
 */
function initialize() {
...
```
* <Marker number={1} /> The method `slide` is wrapped inside a new method `readURL`. If no hash is present, we simply display the first slide as before.


<Admonition type="note" className="congratulations" title="Congratulations!">
**Congratulations, you have written a minimal and operational version of Reveal.js in less than 200 lines of JavaScript code, with just 100 lines of CSS code**. The example presentation is available [here]({{ '/posts_resources/2015-03-03-revealjs-revealed/example.html' | prepend: site.baseurl }}), just like the [complete source code](https://github.com/julien-sobczak/reveal-from-scratch).
</Admonition>
...
```javascript showLineMarkers:{'1': [707], '2': [710], '3': [716], '4': [719]}
...
```
* <Marker number={1} /> Functions are the only possible scope in JavaScript. This is why we use an immediate function to not pollute the global namespace uselessly.
* <Marker number={2} /> We test for a method `define` having a property `amd` to determine if RequireJS is available. Reveal.js declares itself as an anonymous module without any dependency.
* <Marker number={3} /> Like for AMD, we test for an object `exports`. Reveals declares itself like any other Node module.
* <Marker number={4} /> As before, our module is defined as a global variable, or more precisely a property of the object `window`.
