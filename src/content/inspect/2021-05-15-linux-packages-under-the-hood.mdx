...
description: Learn the inner working of Linux packages by rewriting a minimalist version of APT.
tags: [programming]
topics: [linux]
excerpt: >
...
<Admonition type="note" title="What You Need to Know About the Command <code>dpkg</code>">
...
do_build(const char *const *argv)
{
  ...
  dpkg_ar_member_put_mem(ar, "debian-binary", deb_magic, strlen(deb_magic));
...
```c title="src/configure.c" {"1": 14} {"2": 46} {"3": 53-57} {"4": 60}
/**
...
<Admonition type="note" title="What You Need to Know About <code>apt</code>, <code>apt-get</code>, <code>aptitude</code>">
...
_Further documentation_: [man page](https://www.commandlinux.com/man-page/man5/apt.conf.5.html)
</Admonition>
...
```txt title="http://ftp.debian.org/debian/dists/Debian10.9/main/binary-amd64/Packages.gz"
...
```txt title="http://ftp.debian.org/debian/dists/Debian10.9/main/source/Sources.gz"
...
<Admonition type="note" title="What You Need to Know About <code>/var/cache/apt/</code>">
...
<Admonition type="note" title="What You Need to Know About <code>/var/lib/apt/</code>">
...
<dl className="markers">
  ...
  <dt><Marker number={2} /></dt>
  <dd>Concrete values will be replaced in the function `ListSingleVersion` by replacing `${Package}`, `${Origin}`, … by their real values.</dd>
...
bool DoInstall(CommandLine &CmdL)
{
   ...
   if (Cache->InstCount() != verset.size())
...
```txt title="/var/lib/apt/lists/deb.debian.org_debian_dists_buster_main_binary-amd64_Packages"
...
Package: ngraph-gtk
Version: 6.09.01-1
