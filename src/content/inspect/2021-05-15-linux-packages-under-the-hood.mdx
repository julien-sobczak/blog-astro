...
* [How to create an archive manually](#xref:sect-part1).
...
* [5 reasons why a Debian package is more than a simple file archive, Raphaël Hertzog](https://raphaelhertzog.com/2010/11/08/5-reasons-why-a-debian-package-is-more-than-a-simple-file-archive/)
...
You can also learn more about Debian packages by installing a Debian package 😀 (the PDF is also available [online](https://www.debian.org/doc/manuals/packaging-tutorial/packaging-tutorial.en.pdf)):
...
`-- usr
    `-- bin
        `-- hello
```
* <Marker number={1} /> The first version of our package `hello` contains only the binary `hello` built previously and a DEB822 file `control` with the package metadata.
* <Marker number={2} /> We also append basic maintainer scripts that displays a message in the console so that we will know when the installation process runs them.
...
```
* <Marker number={1} /> The package will fail most linter checks. Indeed, we ignored many of the best practices that higher-level commands ensure but we will still be able to install this package on our server.
...
```
* <Marker number={1} /> The variable `dir` is the local directory containing the package files to build. The variable `dest` is the optional filename for the final package file and `debar` is the final name as determined by the function `gen_dest_pathname`, which determines a default name if the argument is missing.
* <Marker number={2} /> The function `dpkg_ar_create` creates the archive file named after the variable `debar`.
* <Marker number={3} /> The function `dpkg_ar_put_magic` defines the magic number `!<arch>\n` telling Linux the file is of type `ar`.
* <Marker number={4} /> The function `dpkg_ar_member_put_mem` appends the file `debian-binary` with the content of the variable `deb_magic`.
* <Marker number={5} /> The function `dpkg_ar_member_put_file` appends the file `control.tar` with the content of a temporary file.
* <Marker number={6} /> Same as above for `data.tar`.
* <Marker number={7} /> The function `dpkg_ar_close` is part of the housecleaning logic and simply closes the file descriptor.
...
```
* <Marker number={1} /> The main function iterates over all packages to install and delegates to the function `process_archive` for the unpacking.
* <Marker number={2} /> The function `process_queue` configures all packages that have been unpacked in the previous step. We will explain the differences between these two steps.
...
```
* <Marker number={1} /> Create a temporary directory (commonly `/var/lib/dpkg/tmp.ci/`).
* <Marker number={2} /> Run the command `dpkg --control` to extract the `DEBIAN/` directory into it.
...
```
* <Marker number={1} /> The enum `want` determines the expected action for this package, like `PKG_WANT_INSTALL` for installation, or `PKG_WANT_PURGE` for the removal of the package and its configuration files.
* <Marker number={2} /> The `eflag` is initialized if the parser finds an error in the control file (ex: missing field), and also later during the installation process.
* <Marker number={3} /> The `installed` and `available` fields contain most of the information present in the `control` files concerning a possible installed version of the package and the new version to install.
* <Marker number={4} /> Some fields like `files` are initialized later by other functions like  `db-fsys-files.c#ensure_packagefiles_available`, which reads the file `/var/lib/dpkg/list/hello.list` to populate this field.
* <Marker number={5} /> The `status_dirty` flag is set when the current status of the package changes, for example from `PKG_STAT_UNPACKED` to `PKG_STAT_INSTALLED`.
...
```
* <Marker number={1} /> The function `parsedb` simply reads a file in [Debian RFC822 format](https://man7.org/linux/man-pages/man5/deb822.5.html), the format we used to write the `control` file.
...
```
* <Marker number={1} /> The status read during parsing is reused to determine if the package is already installed.
* <Marker number={2} /> Update the package status to keep trace that the package has been partially installed. The status will be changed several times during the installation. The function `modstatdb_note` persists the new state to disk.
* <Marker number={3} /> `maintscript_fallback` and `maintscript_installed` delegates to `maintscript_exec` defined in the same file `src/script.c`. This function runs the script in a fork process and aborts if the return code is greater than 0. Differences between the various calls are explained in the next step.
...
<a name="wynk-conffiles"></a>We still haven't talked about conffiles. When upgrading a package, you want the package manager to overwrite the previous version of the files, except for configuration files. You don't want to lose your customizations, don’t you?
...
```
* <Marker number={1} /> Edit the file `/var/lib/dpkg/info/hello.list`.
* <Marker number={2} /> Copy all files under `DEBIAN/` into `/var/lib/dpkg/info/` by prefixing them with the package name `hello.`.
* <Marker number={3} /> Edit the file `/var/lib/dpkg/info/hello.md5sums`.
* <Marker number={4} /> Update the field `Status` in `/var/lib/dpkg/status` for the package `hello` to set the value `install ok unpacked`.
...
int sincenothing = 0;
```
* <Marker number={1} /> The global variable containing the packages to configure.
* <Marker number={2} /> These variables control the algorithm that decides which package must be configured first, which must be postponed, and when to abort the installation completely.
...
```
* <Marker number={1} /> The function `deferred_configure` is the main function doing the configuration and is the subject of the next step.
...
```
* <Marker number={1} /> In case of a missing dependency, the installation will abort only at this step, after the unpacking of the package files.
* <Marker number={2} /> The function `deferred_configure_conffile` renames the conffiles still ending with the suffix `.dpkg-new` created during the unpacking. This function also shows the confirmation prompt.
* <Marker number={3} /> Run the `postinst` maintainer script.
* <Marker number={4} /> Change the status to `PKG_STAT_INSTALLED` and force the update in the `status` database file.
...
_Further documentation_: [apt-get, aptitude, … pick the right Debian package manager for you, Raphaël Hertzog](https://raphaelhertzog.com/2011/06/20/apt-get-aptitude-%e2%80%a6-pick-the-right-debian-package-manager-for-you/)
</Admonition>
...
```
* <Marker number={1} /> The `Read*` methods parse the sources files. We omit the parsing code for brievity but both parsers pushes a new instance of `debReleaseIndex` in the `SrcList`.
...
ListUpdate(Stat, *List);
```
* <Marker number={1} /> `AcqTextStatus` is used to report progress of the files downloading.
...
* <Marker number={2} /> Most APT commands tries to acquire a lock to prevent two processes using the lib APT to run at the same time. The lock file is `/var/lib/apt/lists/lock` but other lock files exists for example to update the APT cache.
* <Marker number={3} /> The method `GetIndexes()` creates new items to download `InRelease` files using the Acquire system.
* <Marker number={4} /> The function `AcquireUpdate()` collects the results from the `Fetcher` and update the cache.
...
```
* <Marker number={1} /> The cache is stored in `/var/cache/apt/pkgcache.bin` and `/var/cache/apt/srcpkgcache.bin`. There are binary files that are loaded in memory.
* <Marker number={2} /> The method `CheckValidity` loads each cache file in memory and checks that they are up-to-date, by verifying that every required index files for every source exists.
* <Marker number={3} /> If both cache files are correct, we can returns immediately. Otherwise, we need to rebuild from scratch the ones that are not fine.
...
```
* <Marker number={1} /> The operator `[]` is overloaded in `pkgDepCache` to return `PkgState[I->ID]`, which is a struct `StateCache` containing the current installed and candidate versions.
* <Marker number={2} /> The method `Upgradable()` reads the state to determine if a new candidate version is available and increments a counter.
* <Marker number={3} /> The macro `P_` is defined by `define P_(msg,plural,n) (n == 1 ? msg : plural)`.
...
```
* <Marker number={1} /> The function `CacheFile.GetPkgCache()` delegates to the method `BuildCaches()` we covered in the previous section about `apt update`. This method is responsible to build the APT cache.
* <Marker number={2} /> Concrete values will be replaced in the function `ListSingleVersion` by replacing `${Package}`, `${Origin}`, … by their real values.
* <Marker number={3} /> The real implementation uses the type `LocalitySortedVersionSet` which is a list ordering packages based on their names in the `Translation` files of the user locale.
...
```
* <Marker number={1} /> The command `apt list --installed` searches for installed packages.
* <Marker number={2} /> The command `apt list --upgradable` searches for installed packages that can be upgraded.
* <Marker number={3} /> The command `apt list --all-versions` searches for all packages in the APT cache.
...
```
* <Marker number={1} /> The function ignores which fields are present in the output format and thus will try to replace all of them. If a field is missing, the replacement will do nothing.
* <Marker number={2} /> The code uses the state information present in `depPkgCache` to determine if the package is installed, or upgradable, and so on.
* <Marker number={3} /> The code ensures no remaining braces are left.
...
```
* <Marker number={1} /> The package problem resolver is launched during step 2 and can add new packages to install to satisfy dependencies. Therefore, the number of packages to install can be different from the number of packages specified in the command line.
...
```
* <Marker number={1} /> Add one to `CmdL.FileList` to skip the `install` command name.
* <Marker number={2} /> Mark this package version to be installed.
* <Marker number={3} /> Ensure the resolver fixed the broken packages before continuing the installation.
...
```
* <Marker number={1} /> APT acquires a lock using the [`fcntl()` system call](https://man7.org/linux/man-pages/man2/fcntl.2.html) which is used to manipulate file descriptors. When called using the flag `F_SETLK`, the call returns -1 if the lock is already held by another process.
* <Marker number={2} /> APT supports multiple package managers but the default is the `dpkg` command. APT uses the class `debSystem` and the associated `pkgDPkgPM` to interact with the `dpkg` command.
...
* <Marker number={4} /> APT asks for confirmation before proceeding to the installation, except if you use options like `apt -y install`.
* <Marker number={5} /> Unlock Dpkg lock `/var/lib/dpkg/lock` to make sure the `dpkg` command can use it.
* <Marker number={6} /> The package manager reads the `/var/lib/dpkg/status` to found out the packages that were removed because none of their files was referenced by another package.
...
```
* <Marker number={1} /> The package manager keeps a list of actions to perform.
* <Marker number={2} /> The method `Install` simply appends a new item in `List` of type `Install`.
* <Marker number={3} /> The method `Go` reads the list of actions and execute them.
...
```
* <Marker number={1} /> The code is a classic example of Linux programming. The code uses the system calls `fork()`, `exec()`, and `wait()` to delegate to the command `dpkg`.
...
```
* <Marker number={1} /> Declare a required dependency available in the standard Debian repository.
* <Marker number={2} /> Use the binary installed by this dependency.
...
```
* <Marker number={1} /> We use the command `dpkg` but we could also have used our Go version created in the first part.
