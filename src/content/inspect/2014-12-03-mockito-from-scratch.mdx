...
```java showLineMarkers:{'1': [71], '2': [79], '3': [86]}
...
```
* <Marker number={1} /> We instantiate a new mock for each test.
* <Marker number={2} /> We configure this mock to associate an instance of `DataSource` with the name `"dataSource"`.
* <Marker number={3} /> We check that the registry has been called once with this name.
...
```
* <Marker number={1} /> When calling a method on a mock (when using `when` or `verify`), Mockito requires only literal values/variables, or only matchers. Under the hood, Mockito makes sure to only work with matchers. This is the role of the method `argumentsToMatchers`.
...
```java showLineMarkers:{'1': [586], '2': [587], '3': [589], '4': [593]}
...
```
* <Marker number={1} /> `stubbed` contains all method calls recorded using `when`.
* <Marker number={2} /> `invocationForStubbing` contains the current method call. We ignore for now the context (`when`, normal call, or `verify`).
* <Marker number={3} /> `registeredInvocations` contains all the effective method calls (calls happening outside a call to `when` or `verify`).
...
```java showLineMarkers:{'1': [702], '2': [707], '3': [712], '4': [723]}
...
```
* <Marker number={1} /> Each `MockHandler` is associated with an instance of a mock. It's the best place to create the instance of `InvocationContainer`.
* <Marker number={2} /> We dequeue all matchers to create an instance of `InvocationMatcher`.
* <Marker number={3} /> We record a possible stubbing (will be confirmed later, or not).
* <Marker number={4} /> If an answer has already been programmed, we simply return it.
